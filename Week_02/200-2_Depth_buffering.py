...
def display():
    ...
    glEnd()
    glutSwapBuffers() #add glutSwapbuffers for double buffer
    glFlush()
...
def my_init():
    ...
    df = pd.read_csv("../models/monkey.tri", delim_whitespace=True, comment='#',
                     header=None, dtype=np.float32)
    centroid = df.values[:, 0:3].mean(axis=0)
    bbox = df.values[:, 0:3].max(axis=0) - df.values[:, 0:3].min(axis=0)

    glEnable(GL_DEPTH_TEST) #add glEnable depth_test
...
def main():
    glutInit(sys.argv)
    glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE | GLUT_DEPTH) # chang single and add depth
    glutInitWindowSize(800, 800)
    glutCreateWindow("Depth Framebuffer Exercise")
