...
out vec3 fPos, eye_position, fNor, fCol;
out vec4 fSha;   // Shadow-space (Light-space) position
void main()
{
    gl_Position = proj_mat * view_mat * model_mat * vec4(position, 1);
    fPos = (model_mat * vec4(position, 1)).xyz;
    eye_position = (inverse(view_mat) * vec4(0, 0, 0, 1)).xyz;
    mat4 adjunct_mat = transpose(inverse(model_mat));
    fNor = (adjunct_mat * vec4(normal, 0)).xyz;
    fSha = shadow_mat * vec4(position, 1);
    fCol = color;
}
'''
    render_frag_code = b'''
#version 140
in vec3 fPos, eye_position, fNor, fCol;
in vec4 fSha;
...
void main()
{
    ...
    vec3 L = normalize(light_position - fPos);
    float LdotN = max(0, dot(L, N));
    vec3 color = fCol * light_intensity * LdotN;
    vec3 V = normalize(eye_position - fPos);
    vec3 H = normalize(L + V);
    float HdotN = max(0, dot(H,N));
    color += Ks * light_intensity * pow(HdotN, shininess);
    float visibility = 1.0;
    //if(texture(shadow_map, fSha.st/fSha.w).r < fSha.z/fSha.w)
        //visibility = 0.1;
    float bias = 0.005;
    if(texture(shadow_map, fSha.st/fSha.w).r < fSha.z/fSha.w - bias)
        visibility = 0.1;
    gl_FragColor = vec4(color * visibility + Ka, 1);
...
'''
    ...
    global shadow_tex_id, shadow_FBO,shadow_map_size
    glActiveTexture(GL_TEXTURE0)
